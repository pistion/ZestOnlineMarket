...
function faunaToast(msg){
  let t = fauna$('.fauna-toast');
  if(!t){
    t = document.createElement('div');
    t.className = 'fauna-toast';
...
(function faunaSetupSettingsNav(){
  ...
  settingsLink.addEventListener('click', (e)=>{
    e.preventDefault();
    const templateKey = document.body.dataset.storeTemplateKey || 'art';
...
(function faunaSetupViewDetailsRedirects(){
  ...
  detailBtns.forEach(btn=>{
    btn.addEventListener('click', (e)=>{
      ...
      window.location.href = document.body.dataset.productPath || '/marketplace';
    });
  });
})();
...
(function faunaSetupGlobalFeedRedirect() {
  ...
  links.forEach(link => {
    link.addEventListener('click', e => {
      e.preventDefault();
      window.location.href = document.body.dataset.publicStore === '1'
        ? '/marketplace'
        : '/seller/dashboard';
    });
  });
